...
function formatPhone(tel) { const d = tel.replace(/\D/g, ''); return d.startsWith('55') ? d : '55' + d; }
...
module.exports = async function handler(req, res) {
  ...
  if (!agendamento || !agendamento.clienteTelefone) return res.status(400).json({ ok: false, error: 'Telefone do cliente ausente.' });
  try {
    const response = await fetch(`${EVOLUTION_URL}/message/sendText/${EVOLUTION_INSTANCE}`, {
      method: 'POST',
      headers: { 'apikey': EVOLUTION_API_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ number: formatPhone(agendamento.clienteTelefone), text: buildMsg(agendamento), options: { delay: 1200 } }),
    });
    const data = await response.json();
    if (!response.ok) return res.status(502).json({ ok: false, error: data?.message || 'Erro na Evolution API' });
