...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (response.status === 409) return res.status(200).json({ ok: true, hint: 'Instância já existe. Gere o QR Code para conectar.' });
    if (!response.ok) return res.status(502).json({ ok: false, error: data?.message || 'Erro ao criar instância' });
